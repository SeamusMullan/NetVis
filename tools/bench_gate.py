...
def check_comparable(baseline, current):
    ...
    # A baseline is only meaningful against the machine class that produced it.
    #
    # This was learned the hard way: the first committed baseline was recorded on
    # a 16-core workstation and the CI runner has 2 cores, so EVERY stage came in
    # 15-60% slower — including stages the branch never touched — and the gate
    # reported a wall of REGRESSION rows for what was purely a change of machine.
    # No threshold fixes that: one wide enough to absorb a hardware difference is
    # too wide to catch a real regression, which would make the gate decorative.
    #
    # So core count is treated like schema and build type: a difference makes the
    # two runs INCOMPARABLE (exit 2, "could not judge"), not a failure (exit 1,
    # "the code got slower"). That distinction is the whole point of having two
    # exit codes. The practical consequence is that the committed baseline must be
    # generated BY CI, and a developer comparing locally will be told to
    # re-baseline rather than shown a misleading pass or fail.
    base_cores = baseline.get("hardware_concurrency", 0)
    cur_cores = current.get("hardware_concurrency", 0)
    if base_cores and cur_cores and base_cores != cur_cores:
        raise GateError(
            "hardware mismatch: baseline was recorded on a {}-core machine, this "
            "run is on {} cores. Wall-clock numbers are not comparable across "
            "machine classes, so this is reported as 'cannot judge' rather than "
            "as a regression. The committed baseline is generated by CI; to "
