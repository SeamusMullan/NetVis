...
// in flat_tensors. Asserts two F32 tensors, correct shapes, a real payload
// offset, and zero payload reads (offsets recorded, bytes untouched — §2.1).
...
namespace {
const char* kFixture = "tests/fixtures/model.safetensors";
}

TEST_CASE("SafeTensors: two F32 tensors, tensor-table mode, no payload reads") {
  ...
  // "__metadata__" is not a tensor; expect exactly w and b.
